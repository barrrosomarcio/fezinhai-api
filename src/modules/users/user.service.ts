    ...
    return this.userRepository.findById(id).pipe(
      ...
      switchMap((user) => {
        ...
        return this.userRepository.update(user).pipe(
          map((updatedUser) => {
            if (!updatedUser)
              throw HttpErrors.notFound(`Failed to update user with ID ${id}`);
            return updatedUser;
          }),
        );
      }),
    ...
    return this.userRepository.findById(id).pipe(
      ...
      switchMap((user) => {
        ...
        const updatedPreferences = {
          ...
          notifications:
            preferencesDto.notifications ?? currentPreferences.notifications,
        };
          ...
          map((updatedUser) => {
            if (!updatedUser)
              throw HttpErrors.notFound(
                `Failed to update user preferences with ID ${id}`,
              );
            return updatedUser;
          }),
        );
