...
export class DynamoDBService implements OnModuleInit {
  ...
  private startConnection(): void {
    try {
      ...
      if (
        !databaseConfig.credentials.accessKeyId ||
        !databaseConfig.credentials.secretAccessKey
      ) {
  ...
  get(
    tableName: string,
    key: Record<string, DynamoDBValueInput>,
  ): Observable<Record<string, DynamoDBValueInput> | null> {
    return defer(() =>
      of(
        new GetCommand({
          TableName: tableName,
  ...
  query(
    ...
    limit?: number,
  ): Observable<Record<string, DynamoDBValueInput>[]> {
    return defer(() =>
      of(
        new QueryCommand({
          TableName: tableName,
